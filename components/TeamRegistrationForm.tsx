...
const TeamRegistrationForm = ({ userId, userRole, isLightMode = false }: TeamRegistrationFormProps) => {
  ...
  const handleSearch = async (query: string, teamId: number) => {
    ...
    // If I am a RIDER, set form for SKIER (which now means any non-rider).
  ...
  const handleSubmit = async () => {
    ...
    if ((userRole === 'SKIER' || userRole === 'SNOWBOARDER' || userRole === 'BOTH') && !division) {
  ...
  const WAIVER_TEXT = `By agreeing to this waiver and release form for the Whitefish Skijoring Event, PARTICIPANT(S) (undersigned participant(s) or undersigned parent(s) or guardian(s)) agree(s) and fully understand(s) the following:
    ...
    <div className="space-y-6 w-full">
      <div className={`p-6 border ${borderColor} ${hoverBorderColor} transition-colors duration-50 rounded-lg ${bgColor} backdrop-blur-sm space-y-4 relative`}>
        ...
        <p className={`text-sm ${isLightMode ? 'text-black/70' : 'text-white/70'} text-center`}>
          Competitors and Guardians please read entire form carefully before signing. Guardians sign both lines.
          <br />
